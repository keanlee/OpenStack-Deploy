...
$GREEN=====================================================================
  ...
  Congratulation you finished to deploy nova on ${YELLOW}$(hostname)${NO_COLOR}${MAGENTA}
...
#RabbitMQ
    ...
    sed -i "/rabbitmq-config/a\transport_url = rabbit://openstack:RABBIT_PASS@rabbit1:5672,openstack:RABBIT_PASS@rabbit2:5672,openstack:RABBIT_PASS@rabbit3:5672" /etc/nova/nova.conf
    sed -i "s/RABBIT_PASS/$RABBIT_PASS/g" /etc/nova/nova.conf 
...
if [[ ${MGMT_IP} = ${COMPUTE_NODE_IP[${#COMPUTE_NODE_IP[*]}-1]} ]];then 
    ...
    for ips in ${COMPUTE_NODE_IP[*]};do   
    ssh -n root@${ips} "for ips in ${COMPUTE_NODE_IP[*]};do scp /var/lib/nova/.ssh/id_rsa.pub root@${ips}:/var/lib/nova/.ssh/authorized_keys;scp /var/lib/nova/.ssh/* root@${ips}:/var/lib/nova/.ssh/;done;chown -R nova:nova /var/lib/nova/;chmod 700 /var/lib/nova/.ssh;chmod 600 /var/lib/nova/.ssh/authorized_keys"
    done
