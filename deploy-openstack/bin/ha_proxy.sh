...
#
mysql -uroot -p$GALERA_PASSWORD -e "GRANT PROCESS ON *.* TO 'clustercheck_user'@'localhost' IDENTIFIED BY 'clustercheck';FLUSH PRIVILEGES;"
...
if [[ $(hostname) = ${CONTROLLER_HOSTNAME[2]} ]];then 
    ...
    if [[ $(gluster volume info all | sed -n '10,12p' | grep ${CONTROLLER_HOSTNAME[0]} | wc -l) -eq 1 ]] && [[ $(gluster volume info all | sed -n '10,12p' | grep ${CONTROLLER_HOSTNAME[1]} | wc -l) -eq 1 ]] && [[ $(gluster volume info all | sed -n '10,12p' | grep ${CONTROLLER_HOSTNAME[2]} | wc -l) -eq 1 ]];then 
        debug "0"
    else 
    ...
    ssh -n root@${CONTROLLER_IP[0]} "echo \"127.0.0.1:vol-1  ${GLUSTERFS_SHARE_DIR} glusterfs defaults,_netdev,noauto,x-systemd.automount 0 0\" >> /etc/fstab"
    
    ssh -n root@${CONTROLLER_IP[1]} "mount -t glusterfs $(hostname):/vol-1 ${GLUSTERFS_SHARE_DIR}"
    ssh -n root@${CONTROLLER_IP[1]} "echo \"127.0.0.1:vol-1  ${GLUSTERFS_SHARE_DIR} glusterfs defaults,_netdev,noauto,x-systemd.automount 0 0\" >> /etc/fstab"
...
case $1 in 
    ...
    HA_ALL)
       Galera
       load_balancing
       if [[ ${GLUSTERFS_USE_FOR_CONTROLLER} = "yes" ]];then 
           Gluster_FS
       elif [[ ${GLUSTERFS_USE_FOR_CONTROLLER} = "no" ]];then
           debug "notice" "No GlusterFS Install For Glance Image Save"
       else
           debug "warning" "Unsupport your Glance images store HA choose:${GLUSTERFS_USE_FOR_CONTROLLER}"
       fi
       ;;
    *)
